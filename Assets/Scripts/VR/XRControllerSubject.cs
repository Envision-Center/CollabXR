...
namespace CollabXR.VR
{
	...
	public class XRControllerSubject : MonoBehaviour
	{
		...
		private Action VisionOSTapEvent,
			VisionOSReleaseEvent;
		...
		private bool VisionOSTapValue,
			VisionOSReleaseValue;

		private List<InputFeatureUsage<bool>> binaryFeatures = new();
		public InputDevice Controller;
		public VisionOSInput VisionOSController;
		...
		private void Update()
		{
			...
			{
				UpdateHandTrackingValues();
				InvokeHandTrackingEvents();
			}
		}
		...
		public bool GetJoystickFlick(InputFeatureUsage<Vector2> feature, JoystickDirection dir, float deadzone = 0.5f)
		{
			...
			Vector2 currentValues = Vector2.zero,
				previousValues = Vector2.zero;
			if (Controller.isValid)
			{
		...
		public void SubscribeToVisionOSTap(Action action)
		{
			VisionOSTapEvent += action;
		}

		public void SubscribeToVisionOSRelease(Action action)
		{
			VisionOSReleaseEvent += action;
		}

		public void UnsubscribeToVisionOSTap(Action action)
		{
			VisionOSTapEvent -= action;
		}

		public void UnsubscribeToVisionOSRelease(Action action)
		{
			VisionOSReleaseEvent -= action;
		}
			...
			if (timeSinceLastSearch >= 0.5f)
			{
				...
				if (DeviceMatches != null && DeviceMatches.Count > 0)
				{
					if (DeviceMatches.Count > 1)
						Debug.LogWarning("XR Controller: Multiple devices matching the device characteristics were found on " + gameObject.name + ". Defaulting to first valid device found.");
		...
		private void UpdateHandTrackingValues()
		{
#if UNITY_VISIONOS
			VisionOSTapValue = VisionOSController.GetPointerState(DeviceCharacteristics) == VisionOSInput.PointerFeatureState.Tapped;
			VisionOSReleaseValue = VisionOSController.GetPointerState(DeviceCharacteristics) == VisionOSInput.PointerFeatureState.Released;
#endif
		}
		...
		private void InvokeHandTrackingEvents()
		{
			if (HardwareConfig.IsVisionOS)
			{
				if (VisionOSTapValue)
				{
					if (!hand.TestIfOverUI())
					{
						Debug.Log($"Tap!");
						VisionOSTapEvent?.Invoke();
					}
				}
				else if (VisionOSReleaseValue)
				{
					if (!hand.TestIfOverUI())
					{
						Debug.Log($"Release!");
						VisionOSReleaseEvent?.Invoke();
					}
				}
			}
		}
