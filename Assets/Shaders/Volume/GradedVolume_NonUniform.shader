Shader "Unlit/GradedVolumeNonUniform"
{
	Properties
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				for (int i = 0; i < _MaxSteps; i++)
				{
					...
					{
						float3 adjustedSamplePosition = samplePosition;
						adjustedSamplePosition.z = pow(adjustedSamplePosition.z+0.5f, 2);
						//adjustedSamplePosition.z = 0.8f;
						float density = _MainTex.Sample(sampler_MainTex, adjustedSamplePosition + float3(0.5f, 0.5f, 0)).r;
